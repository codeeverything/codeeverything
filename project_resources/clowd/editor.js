...
(function() {
	...
	//some globals. everyone likes globals...
	var options;
	var canvas;
	var context;
	var lineContext;
	...
	var highlightFindMatches = false;
	
	var keyHandledOnKeyDown = false;
	
	this.setup = function(c, opts) {
		canvas = c;	//get the canvas element
		context = canvas.getContext("2d");	//get the context for the canvas
		
		options = opts;	//these are our options for initialisation
		...
		//setup the syntax engine to some default - document loading will adjust this
		syntax.engine('JS');
		
		
		//setup our tab string
		...
		//apply our dimensions
		canvas.width = width;
		canvas.height = height;
		
		
		
		//canvas cursor. we're editing text, so show an appropriate cursor icon
		canvas.style.cursor = 'text';
		
		//the line canvas is a separate canvas to render line numbers (and breakpoints etc in future).
		lineCanvas = document.getElementById('lineCanvas');	//TODO: We'll almost certainly lose this and have this as part of a single canvas
		...
		context.textBaseline = "top";
	    	context.font = options.font ? options.font:"11pt courier";
		...
		document.getElementById('verticalScroll').style.left = canvas.offsetLeft + canvas.width + 'px';
		...
		document.getElementById('findcountbutton').onclick = function() {
			findstr(document.getElementById('findinput').value);
			alert(find.count);
			return false;
		}
		
		document.getElementById('findhighlight').onclick = function() {
			highlightFindMatches = !highlightFindMatches;
			render(true);
		}
	...
	this.mousewheel = function(e) {	
    	...
    	if(e.shiftKey) {
    		...
    		scrollInc *= 2;	//TODO: This could be a customisable property - scrollInc *= options.fastScroll
    	}
	...
	this.mousemove = function(e) {
		...
		if(mousedown) {
			...
			//first dirty up the lines in the current selection
			var fixedSelection = fixSelection(editorSelection);
			for(var l=fixedSelection.y1; l<=fixedSelection.y2; l++) 
				dirty[l] = true;
			...
			if(clickYPos > editorSelection.y2) dirty[clickYPos-1] = true;	//here we're saying that if we've moved back down a line in the selection, then as well as rendering the new selection we'll need to render the line we just excluded from the selection
		        ...
		        //now mark the lines in the new selection as dirty for attention
		        var fixedSelection = fixSelection(editorSelection);
	...
	this.keydown = function(e) {	
		...
		if(which == 37 || which == 39) {
			...
			if(e.shiftKey) {
			    ...
			    //var fixedSelection = fixSelection(editorSelection);	//fixing the selection breaks the selection... go figure
			    for(var i=editorSelection.y1; i<=editorSelection.y2; i++) dirty[i] = true; //dirty lines within the selection
			    //console.log(editorSelection);
			} else {
	...
	this.loadFileFromString = function(str) {
		//TODO: We need a check that the file isn't currently open
		var file = str;
		var path = 'tempfile.js';
	...
	this.newFile = function(fileType) {
		...
		var fileName = prompt('Give this file a name (will be saved as a JS file)');
		if(!fileName) return;
		
		var fileOverwrite = true;
		$.ajax({
			url:'backend/checkFileExists.php', 
			data: {name: fileName}, 
			type: 'POST',
			async: false,
			success: function(data) {
				//alert(data);
				if(data) {
					fileOverwrite = confirm('The file '+fileName+' already exists, do you want to overwrite it?');
				}
			}
		});
		
		//bail if the file already exists and we don't want to overwrite it
		if(!fileOverwrite) return;
		
		//create a new document and set the currentDoc to it, then add it to the list of open documents
		currentDoc = new Document(this);
		documents.push( currentDoc );
		
		//load the documnet up (its just an empty file
		var path = 'development/'+fileName+'.js';
		currentDoc.loadDocument(" ", path);
		lines = currentDoc.getLines();
		
		//move us back to the top of the document
		currentDoc.cursor.setCursor({x: 1, y: 0});
		offset = 0;	
	...
	var findstr = function(str) {
		...
		for(;i--;) {
			...
			if(idx != -1) {
			    ...
			    //while(result = re.exec(lowerCasedLine)) {
			    while(idx != -1) {
			        ...
			        count++;
			        if(count > 10) { 
			        	//console.log('breaking on count'); break; 
			        }	//a restirction on the number of matches per line? I suppose it makes sense just in case we get into an infinite loop
			    }
			}
		}
	...
	function render(full) {
	    ...
	    */
	    ...
	    if(currentDoc.cursor.pos.y >= y && currentDoc.cursor.pos.y <= maxLine) {
	        ...
	        context.fillRect(0, lineHeight*(currentDoc.cursor.pos.y-offset), canvas.width, lineHeight);
	    }
	    ...
	    //Some selection setup stuff.
	    //This previously lived within the loop over each line, but really we only need to do this once per render, so moved up here (not that rendering was slow anyway)
	    var renderSelection = {};
		...
		//TODO: Simplify this with a call to fixSelection or something
	    ...
	    for(; y<maxLine;y++) {
	        ...
	        */
			...
			if(yHigh != yLow) {
				...
				context.fillRect(x, lineHeight*(y-offset), w, lineHeight);
			}
	        ...
	        if(highlightFindMatches && find.matches) {
		        ...
		        if(y >= yLow && y <= yHigh) {
		            ...
		            if(localMatches[y]) {
		                ...
		                for(var j=0; j<localMatches[y].length; j++) {
		                    ...
		                    context.fillRect(x1, lineHeight*(y-offset), w, lineHeight);
		                }
		            }
	        ...
	        */
	        ...
	        if(!renderMode) {
	            ...
	            context.fillText(text, 10, lineHeight*(y-offset));
	        } else {
	            ...
	            for(var style in localCachedRegions[y]) {
	                ...
	                context.fillText(line, 10, lineHeight*(y-offset));
	                //fillCount++;
	            }
